	...
	for step in range(0, steps + 1):
		...
		for i in range(1, width - 1):  # Ignores the padding.
			for j in range(1, height - 1):  # Ignores the padding.
				...
				localNeigh = ar[j - 1:j + 2,
							 i - 1:i + 2]  # Numpys indexes are [Inclusive, Exclusive), hence the extra +1.

				land = localNeigh[1, 1] == 1  # Checks if the center value, the point of the neighbourhood, is land.

				aliveNeighbours = np.sum(localNeigh) - localNeigh[
					1, 1]  # Sums the array and subtracts the possible extra value.
...
def heightArray(size: (int, int), volat: float, noiseProb: float):
	...
	for x0 in range(2, size[1]):  # Iterates over the first row, initialising it.
		if rng.uniform() < noiseProb:  # This is the chance that some independent randomness is introduced.
			...
			heightArray[0, x0] = ((
										  point1 + point2) / 2) + volat * uni  # Calulates the midpoint and adds a weighted U[-1,1].
...
class Millow():
	...
	def __init__(self, mapType: str):
		...
		possTypes = ['continents', 'dense islands',
					 'sparse islands']  # Possible map types, will be updated as I find more.
		...
		self.size = (
			1080, 1920)  # The pixel size of the final image, currently not variable. Will be in future versions.

		self.mapType = mapType  # Sets the map type. Currently unused.

		self.mapTypeDict = {'continents': (9, 4, 6, 10, 1), 'dense islands': (7, 1, 9, 15, 0),
	...
	def generateRawMap(self):
		...
		rawMap = noiseArray(
			(int(self.size[0] / 50), int(self.size[1] / 50)))  # Size is divided by 50 since it will be zoomed by this
		...
		(a, b, c, d, e) = self.mapTypeDict[
			self.mapType]  # Unpacks the constants used for the smoothing procedure, which
		# produces the desired map type.

		rawMap = cellularSmooth(rawMap, a, b, c, d,
								borderValue=e)  # See above for an explanation of a,b,c,d,e. This smooths the noise
	...
	def generateHeight(self):
		...
		alphaArray = noiseArray(
			(int(self.size[0] / 5), int(self.size[1] / 5)))  # Size is divided by 50 since it will be zoomed by this
		# amount to create smooth features. This uses percolation at the critical probability for features of all sizes.

		alphaArray = cellularSmooth(alphaArray, 9, 2, 6, 17, borderValue=0)  # Dense islands.

		alphaArray = zoom(alphaArray,
						  5)  # Zooming the array restores it to the intended pixel dimensions and smooths out
		# the roughness.

		alphaArray = alphaArray[0:1080, 0:1920]  # Trims any extra entries possibly caused by rounding.

		alphaArray = alphaArray.astype(
			np.float32)  # Changes the datatype so that Gaussian blur and divison work with the
		# desired accuracy.
		...
		alphaArray = ((alphaArray - np.amin(alphaArray)) / np.amax(
			alphaArray)) * 255  # All values will be in the range (0,255).
		...
		rawHeight = np.full((1080, 1920, 4), [255, 255, 255, 0],
							dtype=np.uint8)  # Produces a black RGBA array with zero opacity.
